...
//64 32
public class Main {
    private final static int wid = 400;
    private final static int hei = 236;
	private static int[][] out = new int[hei][wid];
    public static void main(String[] args) {
		...
		int[][] temp = new int[yscale][xscale];
			...
			write(sb2.toString());
			//ImagetoArrayUtils.writeImageFromArray("/home/hakureisino/5.jpg", "jpg", out);
    ...
    public static void write (String content) {
        try {
            String path = "/home/hakureisino/" + File.separator;//要创建的文件的位置
//创建文件夹
            File newdirpath = new File(path);


            File file = new File(newdirpath.getAbsolutePath() + "/" +"m" + ".sh");
            if (file.exists()) {
